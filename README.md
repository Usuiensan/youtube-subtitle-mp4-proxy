...
Web UI の「スライドショー作成」タブで、PDF 1 ファイルまたは PNG・JPEG・WebP の画像複数枚を選び、1 枚あたりの表示秒数を指定して MP4 を作成できます。画像はファイル名の自然順（`1.png`、`2.png`、`10.png`）で処理され、横長・縦長とも 1920x1080 の黒帯付きで縦横比を維持します。PDF はページ順に変換されます。
...
TranslateGemma 12B を使う場合は、別テンプレートを `TRANSLATEGEMMA_PROMPT_TEMPLATE` か `TRANSLATEGEMMA_PROMPT_TEMPLATE_FILE` で差し替えます。既定のリポジトリ管理版は [prompts/translategemma-prompt.txt](/C:/private/youtube-subtitle-mp4-proxy/prompts/translategemma-prompt.txt) です。こちらは `translategemma:12b` 向けの固定型に寄せてあります。
...
export TRANSLATEGEMMA_PROMPT_TEMPLATE_FILE=/home/masato/youtube-subtitle-mp4-proxy/prompts/translategemma-prompt.txt
```
...
Discord の `/prepare` は、URL や動画 ID を1つ入れるだけで動きます。動画 URL に `list=` が付いていて単一動画と再生リストの両方に見える場合は、ボタンで「再生リストとして全部処理」か「動画1件だけ処理」を選びます。
...
単体動画で `lang:ja` を指定し、日本語字幕が存在しない場合は、準備を始める前に翻訳元字幕を選ぶ UI を表示します。候補が1件だけなら翻訳元は固定され、既定の日本語のまま `実行` を押すだけで準備できます。翻訳先は必要な場合だけ変更できます。YouTube の自動生成字幕・自動翻訳字幕は使用しません。
...
字幕が存在しない動画や指定言語の字幕が取得できない動画は `422` を返します。動画情報の取得時に削除、非公開、地域制限、年齢確認、bot判定、PO Token 不足などが疑われる場合は `404` を返し、`yt-dlp` の診断メッセージもレスポンスに含めます。

ブラウザでは視聴できるのに「この動画は利用できません」になる場合は、まず `yt-dlp` を更新し、必要に応じて有効な `cookies.txt` を `YTDLP_COOKIES_FILE` に設定してください。存在しない cookies パスを設定すると起動環境からは認証できないため、API は明確な設定エラーを返します。YouTube 側が PO Token を要求する場合は `YTDLP_EXTRA_ARGS` に `yt-dlp` の `--extractor-args` や player client 設定を追加して切り分けます。
