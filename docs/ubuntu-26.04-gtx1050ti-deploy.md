...
DISCORD_URL_INTAKE_CHANNEL_ID=YOUR_DISCORD_CHANNEL_ID
...
単体動画で `lang:ja` を指定し、日本語字幕が存在しない場合は、準備開始前に Discord の選択 UI で翻訳元字幕と翻訳先を選びます。翻訳プロファイルは `TRANSLATION_DEFAULT_PROFILE` で固定します。Google Cloud Translation と YouTube の自動生成字幕・自動翻訳字幕は使用しません。一括準備では動画ごとの UI は出さず、従来どおり自動選択します。

翻訳元を明示した版は query string ではなく path に含めます。例: `https://YOUR_DOMAIN/youtube/dQw4w9WgXcQ/ja/en`。翻訳プロファイルはURLに含めず、`TRANSLATION_DEFAULT_PROFILE` で固定します。従来の `/youtube/VIDEO_ID/ja` は細かい版を指定しない既定版です。
...
日本語字幕がない動画では、外国語の手動字幕を選んで日本語へ翻訳できます。翻訳workerはジョブごとに起動・終了するため、FastAPI本体へモデルをロードしません。LLMには字幕IDと原文だけを動画1本分まとめて1回送信し、Structured Output / JSON Schemaで連続ID範囲を検証してから、各範囲の開始〜終了時刻を持つ訳SRTへ再結合します。翻訳が終わってworkerが終了してからNVENCを開始します。
