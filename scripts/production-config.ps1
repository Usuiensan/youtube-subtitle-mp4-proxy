...
if ($Set) {
    if ($Key -notmatch '^(LOCAL_LLM_MAX_OUTPUT_TOKENS|TRANSLATION_CHUNK_INPUT_TOKENS|TRANSLATION_DEFAULT_PROFILE|TRANSLATION_API_RETRY_MAX_ATTEMPTS|TRANSLATION_API_RETRY_BASE_SECONDS|GEMINI_THINKING_LEVEL|SYSTEM_METRICS_INTERVAL_SECONDS|SYSTEM_METRICS_HISTORY_SECONDS|DISCORD_OPERATOR_USER_ID|DISCORD_URL_INTAKE_CHANNEL_ID|CACHE_ARCHIVE_MOUNT_POINT)$') { throw 'Key はallowlist内で指定してください。' }
