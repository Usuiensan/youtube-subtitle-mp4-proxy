...
class Settings:
    ...
    slideshow_dir = Path(os.getenv("SLIDESHOW_DIR", str(cache_hot_dir.parent / f"{cache_hot_dir.name}-slideshows")))
    slideshow_ttl_seconds = int(os.getenv("SLIDESHOW_TTL_SECONDS", "86400"))
    slideshow_max_files = max(1, int(os.getenv("SLIDESHOW_MAX_FILES", "500")))
    ...
    subtitle_margin_l = int(os.getenv("SUBTITLE_MARGIN_L", "80"))
    subtitle_margin_r = int(os.getenv("SUBTITLE_MARGIN_R", "80"))
    ...
    translation_default_profile = os.getenv("TRANSLATION_DEFAULT_PROFILE", "gemini_2_5_flash_lite").strip().lower() or "gemini_2_5_flash_lite"
    ...
    translation_audit_api_token = os.getenv("TRANSLATION_AUDIT_API_TOKEN", "").strip()
    translation_config_api_token = os.getenv("TRANSLATION_CONFIG_API_TOKEN", "").strip()
    llm_translation_usage_file = Path(os.getenv("LLM_TRANSLATION_USAGE_FILE", str(cache_hot_dir / "llm-translation-usage.json")))
